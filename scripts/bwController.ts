...
import { getAccountInfo } from '../wrappers/getAccountHelper';
...
const transferAction = async (provider: NetworkProvider, ui: UIProvider) => {
    ...
    const totalTonAmount = forwardAmount + mintFees;
    const getLastSeqno = await getLastBlock(provider);
    const curState = (await getAccountInfo(provider, getLastSeqno, walletContract.address)).account;
...
const withdrawAction = async (provider: NetworkProvider, ui: UIProvider) => {
    ...
    if (gotTrans) {
        ...
        if (isMinterBalanceWithdrew) {
            const senderTonBalance = (await getAccountInfo(provider, getLastSeqno, sender.address!)).account.balance
                .coins;
            ui.write(
...
export async function run(provider: NetworkProvider) {
    ...
    //     }
    // } while (retry);
    // Prompt user for minter address instead of using hardcoded one
    const minterAddress: Address = await promptAddress('Please enter minter address:', ui);
