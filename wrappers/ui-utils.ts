import { getAccountInfo } from './getAccountHelper';
...
export const promptWithdrawAmount = async (prompt: string, provider: UIProvider): Promise<string> => {
    let resAmount: number;
    do {
        let inputAmount = await provider.input(prompt);
        resAmount = Number(inputAmount);
...
export const getAccountLastTx = async (provider: NetworkProvider, address: Address) => {
    const api = provider.api();
    const latestBlock = await getLastBlock(provider);
    
    let response: any;
    if ('getAccountLite' in api) {
        response = await (api as any).getAccountLite(latestBlock, address);
    } else {
        // Fallback for different client versions
        response = await (api as any).getAccount(latestBlock, address);
    }
    
    if (response.account.last == null) {
        throw new Error('Contract is not active');
    }
    return response.account.last.lt;
};
