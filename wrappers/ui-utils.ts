...
import {EnhancedAPIUtils} from "./api-providers";
...
export const getLastBlock = async (provider: NetworkProvider) => {
    try {
        ...
        console.warn('Primary provider failed, using enhanced API with fallback...');
        const isTestnet = provider.network() !== 'mainnet';
        const enhancedAPI = new EnhancedAPIUtils(isTestnet);
        return await enhancedAPI.getLastBlock();
    }
}
...
export const getAccountLastTx = async (provider: NetworkProvider, address: Address) => {
    try {
        ...
        console.warn('Primary provider failed for getAccountLastTx, using enhanced API with fallback...');
        const isTestnet = provider.network() !== 'mainnet';
        const enhancedAPI = new EnhancedAPIUtils(isTestnet);
        const lastBlock = await enhancedAPI.getLastBlock();
        const res = await enhancedAPI.getAccountLite(lastBlock, address);
        if (res.account.last == null)
            throw (Error("Contract is not active"));
        return res.account.last.lt;
    }
...
export const waitForTransaction = async (provider: NetworkProvider, address: Address, curTx: string | null, maxRetry: number, interval: number = 1000) => {
    try {
        ...
        console.warn('Primary provider failed for waitForTransaction, using enhanced API with fallback...');
        const isTestnet = provider.network() !== 'mainnet';
        const enhancedAPI = new EnhancedAPIUtils(isTestnet);
        return await enhancedAPI.waitForTransaction(address, curTx, maxRetry, interval);
    }
}
